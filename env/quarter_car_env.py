...
class QuarterCarEnv(gym.Env):
    ...
    def __init__(
        ...
        # Gauss-Markov yol üretici — sadece random_road için
        self._road_generator = RandomRoadGenerator(
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Yol üreticiyi sıfırla
        self._road_generator.reset()
    ...
    def _road_profile(self, t: float) -> float:
        if self.road_type == "random":
            # Gauss-Markov süreci — zamansal korelasyonlu
            return self._road_generator.step(self.rng)
        ...
        elif self.road_type == "multi_bump":
            return multi_bump_road(t, amplitude=self.road_amplitude)

        elif self.road_type == "real":
            # PVS gerçek veri — _road_profile_array'den oku
            if getattr(self, "_use_array_profile", False) and \
               getattr(self, "_road_profile_array", None) is not None:
                idx = min(self.step_count, len(self._road_profile_array) - 1)
                return float(self._road_profile_array[idx])
            return 0.0
