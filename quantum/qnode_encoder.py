...
class QNodeEncoder(BaseEncoder, nn.Module):
    ...
    def encode_torch(self, state: torch.Tensor) -> torch.Tensor:
        ...
        if self.frozen:
            with torch.no_grad():
                out = self._encode_batch(state, params)
        else:
            out = self._encode_batch(state, params)
    ...
    def _standardize(self, out: torch.Tensor) -> torch.Tensor:
        """Output mean=0, std=1 normalize."""
        mean = out.mean(dim=-1, keepdim=True)
        std  = out.std(dim=-1, keepdim=True).clamp(min=1e-6)
        return (out - mean) / std
